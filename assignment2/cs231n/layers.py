...
def max_pool_forward_naive(x, pool_param):
  ...
  for h in xrange(out_h):
      ...
      for w in xrange(out_w):
          ...
          for c in xrange(C):
              for n in xrange(N):
                  x_sub = x[n,c,sh:sh+ph,sw:sw+pw]
                  out[n, c, h, w] = np.max(x_sub)
    ...
    for h in xrange(out_h):
      ...
      for w in xrange(out_w):
          ...
          for c in xrange(C):
              for n in xrange(N):
                  x_sub = x[n,c,sh:sh+ph,sw:sw+pw]
                  max_x_sub = np.max(x_sub)
                  dx[n,c,sh:sh+ph,sw:sw+pw] += (x_sub == max_x_sub) * dout[n,c,h,w]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return dx
